...
1. 单击重置密码按钮后，在弹出的 UOS ID 重置密码窗口中，设置手机号/邮箱、新密码、重复密码、密码提示、获取验证码，单击 **重置**。
...
UOS ID 注册成功并登录后，才能使用 UOS ID 相关的功能。

如果您尚未注册 UOS ID，可以通过以下方式进行注册。

- 进入统信官方网站进行注册（支持微信扫描、手机号码等注册方式）。
- 控制中心 UOS ID 界面，单击 **登录**，弹出 UOS ID 登录界面，单击 **注册**，弹出 UOS ID 注册界面进行注册（支持手机号码和微信注册方式）。

登录 UOS ID 后，您还可以使用云同步、应用商店、邮件客户端、浏览器等应用的相关云服务功能。

> ![tips](../common/tips.svg) 窍门：当 UOS ID 绑定了本地帐户后，可以使用 UOS ID 重置密码。

开启云同步功能可自动同步各种系统配置到云端，如网络、声音、鼠标、更新、任务栏、启动器、壁纸、主题、电源等。若想在另一台电脑上使用相同的系统配置，只需登录此 UOS ID 帐户，即可一键同步相应的配置到设备。
...
1. 在控制中心首页，单击 ![personalization_normal](../common/personalization_normal.svg)。
2. 单击 **通用**，开启 **窗口特效**。
3. 开启 **窗口移动时启动透明特效**，可实时体验窗口移动时的透明效果。
...
系统支持即时设置时间格式、货币格式和数字格式。

1. 在控制中心首页，单击 ![time](../common/time.svg)。
2. 单击 **格式设置**，可以设置时间格式、货币格式和数字格式。
