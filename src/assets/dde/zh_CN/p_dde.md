...
> ![notes](../common/notes.svg) 说明：禁用无线网络功能时，飞行模式不可用。
...
3. 在插件区域勾选或取消勾选 **回收站**、**系统监视器**、**电源**、**显示桌面**、**屏幕键盘**、**通知中心**、**时间**、**桌面智能助手** 等插件，可以设置这些插件在任务栏上的显示和隐藏效果。
...
- 搜索框：输入关键字，回车，搜索相应设置。
- 主菜单：单击 ![menu](../common/icon_menu.svg) 进入主菜单。在主菜单中，您可以设置窗口主题，查看版本或退出控制中心。
...
3. 选择帐户类型，输入用户名、密码和重复密码，设置全名、密码提示。
4. 单击 **创建**。
...
> ![notes](../common/notes.svg) 说明：管理员帐号过期之后需要先修改密码才能进行授权。
...
密码有效期自修改之日起开始计算。例如：用户A在9月1日修改了密码后，将密码有效期更改为30天。在这种情况下，密码可以在9月1日至9月30日这期间的30天正常使用。
...
Union ID注册成功并登录后，才能使用Union ID相关的功能。

如果您尚未注册Union ID，可以通过以下方式进行注册。

- 进入统信官方网站进行注册（支持微信扫描、手机号码等注册方式）。
- 控制中心Union ID界面，单击 **登录**，弹出Union ID登录界面，单击 **注册**，弹出Union ID注册界面进行注册（支持手机号码和微信注册方式）。

登录Union ID后，您还可以使用云同步、应用商店、邮件客户端、浏览器等应用的相关云服务功能。

> ![tips](../common/tips.svg) 窍门：当Union ID绑定了本地帐户后，可以使用Union ID重置密码。

开启云同步功能可自动同步各种系统配置到云端，如网络、声音、鼠标、更新、任务栏、启动器、壁纸、主题、电源等。若想在另一台电脑上使用相同的系统配置，只需登录此Union ID帐户，即可一键同步以上配置到设备。
...
通过个人热点将计算机网络信号转换为WiFi热点，以供一定距离内的其他设备无线连接。要开启无线热点，计算机必须装有支持热点功能的无线网卡。
...
3. 选择VPN协议类型，并输入名称、网关、用户名、密码等信息。（导入VPN会自动填充信息）
...
3. 进入“我的设备”选择已配对连接的蓝牙设备，单击已连接附近的按钮 ![next](../common/next.svg) 进入详情，单击 **发送文件** 按钮（仅PC/手机/平板的蓝牙设备有该按钮），在弹出的文件管理器窗口中选择要发送的文件，单击 **打开**。
...
通过拖曳低电量滑块，调节低电量的阈值；或通过拖曳自动待机电量滑块，调节待机电量的阈值。
...
在更新设置页面开启可从仓库更新类型中的“系统”、“仅安全更新”或“第三方仓库“后，自动检查、自动下载、自动安装、更新提醒才能正常操作。
...
> ![notes](../common/notes.svg) 说明：arm架构系统仅支持grub验证，mips架构系统暂不支持grub验证、不支持更换启动菜单背景图。
...
> ![notes](../common/notes.svg) 说明：amd、arm架构系统支持域管理设置功能。
