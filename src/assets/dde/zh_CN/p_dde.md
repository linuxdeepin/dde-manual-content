...
> ![notes](../common/notes.svg) 说明：关于显示的设置，具体操作请参考 [显示设置](#显示) 。
...
开启“禁用自由调节”后，可锁定任务栏高度，避免误拖拽改变任务栏大小。
...
3. 在插件区域勾选或取消勾选 **回收站**、**系统监视器**、**关机**、**显示桌面**、**屏幕键盘**、**通知中心**、**时间**、**截图录屏**等插件，可以设置这些插件在任务栏上的显示或隐藏效果。
...
> ![notes](../common/notes.svg) 说明：在触控板上，四指单击，显示/隐藏启动器， 对应 **Super** 快捷键。
...
![multi|697](fig/p_multiScreen.png)
...
扩展模式下，当多个显示器拼接为矩形布局时，可以启用跨屏拼接功能。跨屏拼接将多个物理显示器组合为一个连续的逻辑显示区域，实现单桌面跨屏显示。

1. 在多屏设置区域，选择 **扩展** 模式。
2. 调整屏幕布局，确保多个屏幕拼接为矩形状态。
3. 开启 **跨屏拼接** 开关。

> ![notes](../common/notes.svg) 说明：跨屏拼接模式下，亮度调节功能可用，但屏幕拖动、分辨率、刷新率、方向和缩放等设置将被禁用。热插拔显示器或切换多屏模式时，系统会自动退出跨屏拼接模式。
...
> ![tips](..\common\tips.svg) 窍门：通常，需要调大输入音量，确保能够听到声源的声音，但是音量不宜过大，因为这会导致声音失真。可以对着输入设备如麦克风以正常说话的音量讲话，并观察反馈音量的变化，变化较明显，则说明输入音量合适。
...
> ![tips](..\common\tips.svg) 窍门：您可以单击播放按钮试听音效。
...
> ![notes](..\common\notes.svg) 说明：您还可以打开 **自动同步配置**、设置服务器同步参数。也可填加其他时区服务器地址，进行自动同步。
...
1. 在控制中心侧边栏选择 **系统**，单击 **时间和日期**。
2. 单击时区列表中 **添加** 按钮 ，通过搜索选择时区。
3. 单击 **添加**。

##### 删除时区

1. 在控制中心侧边栏选择 **系统**，单击 **时间和日期**。
2. 单击时区列表后面的 **删除** 按钮 ，删除已添加的时区。
...
> ![notes](..\common\notes.svg) 说明：arm 架构系统仅支持 grub 验证，mips 架构系统暂不支持 grub 验证、不支持更换启动菜单背景图。
...
> ![tips](..\common\tips.svg) 窍门：您也可以把图片拖拽到预览窗口，来更换启动菜单的背景。
...
在控制中心侧边栏选择 **系统**。选择 **开发者选项**，单击 **进入开发者模式** 按钮，在弹出的授权认证窗口中输入认证信息，认证通过，即可进入开发者模式。
...
在该区域还可以开启或关闭 **磐石只读保护**；关闭后会解锁系统目录，可能导致系统损坏，请谨慎操作。
...
> ![notes](..\common\notes.svg) 说明：amd、arm 架构系统支持域管理设置功能。
...
**激活方式**分在线激活（试用期激活、输入序列号激活、导入激活文件激活）和离线激活。
...
- 如果当前激活服务器出现异常或崩溃了，通过授权管理界面，单击主菜单中的设置，可以自定义新的激活服务器。
...
> ![tips](..\common\tips.svg) 窍门：您可以单击任务栏的快捷面板，设置、查看当前网络状态。
...
> ![notes](..\common\notes.svg) 说明：打开 **仅用于相对应的网络上的资源** 开关，可以不将 VPN 设置为默认路由，只在特定的网络资源上生效。
...
2. 单击 **桌面和任务栏**，进入桌面和任务栏设置界面，您可以：
   - 选择任务栏模式为经典模式或居中模式。
   - 通过拖拽任务栏大小滑块改变任务栏大小。
   - 打开或关闭 **禁用自由调节**，锁定任务栏高度，避免误拖拽改变任务栏大小。
   - 设置位置为上、下、左或右。
   - 设置显示状态为一直显示、一直隐藏或智能隐藏。
   - 打开或关闭 **合并应用图标**，设置同一应用的多个窗口是否合并为一个任务栏图标显示。
...
3. 双击想连接的蓝牙设备，或点击**连接**，在该设备上输入蓝牙配对码（若需要），配对成功后将自动连接。
...
1. 在控制中心侧边栏选择 **蓝牙和其他设备**，单击 **鼠标与触控板**。
...
2. 在 **通用** 区域设置：
 ...
 > ![ ](..\common\notes.svg)说明：开启左手模式后，鼠标和触控板的左右键功能互换。
...
4. 在**指针大小**区域，单击选择 小 / 中 / 大 ，调整光标大小。
5. 开启 **鼠标加速**，调整鼠标移动响应速度。
6. 开启 **插入鼠标时禁用触控板**，在接入鼠标后触控板处于禁用状态。
7. 单击 **自然滚动** 开关，开启相应功能。

> ![notes](..\common\notes.svg) 说明：
> 
> - 开启鼠标加速，提高了指针精确度，鼠标指针在屏幕上的移动距离会根据移动速度的加快而增加。可以根据使用情况开启或关闭。
> - 当没有触控板时，不会显示 “插入鼠标时禁用触控板”。
...
在此模块可以设置键盘属性，以便符合个人的输入习惯，设置输入法，以及自定义快捷键。

![0keyboard](fig\keyboard.png)

##### 键盘属性

1. 在控制中心边栏选择 **蓝牙和其他设备**，单击 **键盘** 。
2. 在 **通用** 区域设置。
3. 调节 **重复延迟**/**重复速度**。
...
1. 在控制中心侧边栏选择 **蓝牙和其他设备**，单击 **键盘** 。
2. 单击 **输入法**，进入输入法界面。
3. 单击 **添加输入法** 按钮 ，您可以：
   - 单击 **前往商店下载**，在应用商店下载输入法，当该输入法下载完成后，系统自动设置其为默认输入法。
   - 进入添加输入法界面，选择输入法，将该输入法添加到输入法管理列表。

> ![notes](..\common\notes.svg) 说明：在输入法管理界面，单击 **编辑** 后，单击 **删除** 按钮即可移除某输入法。对于被移除（非卸载）的输入法，可以在添加输入法界面进行添加。
...
> ![notes](..\common\notes.svg) 说明：在输入法高级设置界面中，还可以设置更多快捷键。
...
> ![tips](..\common\tips.svg) 窍门：若要禁用快捷键，请按下键盘上的 **Backspace** ![Backspace](..\common\Backspace.svg) 键。若要取消修改快捷键，按下 **Esc** 键， 或单击下方的“恢复默认”按钮。
...
> ![tips](..\common\tips.svg) 窍门：若要修改快捷键，单击输入新的快捷键即可。若要修改自定义快捷键的名称和命令，单击“自定义快捷键”后的 **编辑** ，单击快捷键名称后的 ![edit](..\common\edit.svg)，进入修改页面。

#### 触控屏

当接入触控屏时，您可以设置所在屏幕，并对其进行调整。

1. 在控制中心侧边栏选择 **蓝牙和其他设备**，单击 **触控屏**。
2. 对屏幕进行设置后，单击 **确定**。
...
> ![notes](..\common\notes.svg) 说明： 连接数位板设备后，该模块才会显示。

![0graphicstablet](fig\graphics-tablet.png)

1. 在控制中心侧边栏选择 **蓝牙和其他设备**，单击 **数位板** 。
2. 选择 **笔** 模式。
3. 调节 **压感力度**。
...
- 在隐私和安全设置界面，您可以设置应用访问摄像头、文件和文件夹的权限。系统默认情况下，所有应用都可以访问摄像头、文件和文件夹；针对文件和文件夹模块，系统目前支持对“图片”文件夹和“文档”文件夹的权限管控。

- 当权限开启时，应用可以正常访问相关权限；当权限关闭时，应用将无法访问相关权限，系统会提示您前往控制中心-隐私和安全模块开启权限，或者不予处理。

![](fig/PrivacyandSecurity.png)

#### 摄像头

- 支持管理单个应用对摄像头的访问权限，单击![](../common/normal.svg)关闭权限，或者单击![](../common/un.svg)重新开启权限。

#### 文件和文件夹

- 支持分别管理单个应用对“图片”文件夹和“文档”文件夹的访问权限，单击![](../common/normal.svg)关闭权限，或者单击![](../common/un.svg)重新开启权限。
...
1. 拖曳窗口到屏幕左侧或右侧，当光标接触到屏幕左右侧边缘时，会出现分屏提⽰动效，动效颜色与活动色一致，释放窗口，窗口将吸附在屏幕左半边或右半边。
   ...
   > ![notes](../common/notes.svg) 说明：当应用支持分屏时，将光标悬浮于应用窗口的最大/还原按钮上，在弹出的菜单中选择 ![split_screen](../common/split_hover_2.svg) ，即可进入分屏。
