...
在剪贴板上，可以将有源图片、文件类信息直接拖拽到桌面保存，也可以将文字、字符等信息拖拽到可编辑文本中。
...
1. 在控制中心首页，单击 ![account_normal](../common/account_normal.svg)。
2. 单击当前帐户。
3. 单击安全问题所在行的图标![next](../common/next.svg) ，进入安全问题设置页面。
4. 选择3个不同的安全问题并设置答案后，单击 **确定**。
...
个人热点将电脑连接的网络信号转换为WiFi热点，以供一定距离内的其他设备无线上网。要开启无线热点，电脑必须连接到网络并装有支持热点功能的无线网卡。
...
1. 拖拽窗口到屏幕左侧或右侧，当光标接触到屏幕左右侧边缘时，会出现分屏提⽰动效，动效颜色与活动色一致，释放窗口，窗口将吸附在屏幕左半边或右半边。
...
2. 在另一半未被占据的屏幕中，会出现其他所有窗口的预览，单击其中一个使其显示在另一半屏幕中，实现分屏浏览。也可以把其他窗口拖拽到屏幕两边，叠加分屏窗口。
...
分屏浏览时，将光标移动到屏幕中间两个窗口的连接处进行左右拖拽，可以同时修改两个窗口的宽度。
