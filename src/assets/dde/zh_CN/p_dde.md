...
成功登录系统后，即可体验统信 UOS 桌面环境。桌面环境主要由桌面、任务栏、启动器、控制中心和窗口管理器等组成，是您使用该操作系统的基础。
...
1. 在桌面上，单击鼠标右键。
2. 单击 **壁纸与屏保**，选择 **屏保**，在桌面底部预览所有屏保。
3. 选中一个屏保，设置闲置时间。
4. 勾选 **恢复时需要密码**，以便更好地保护您的隐私。
5. 单击 **设置屏保**，待电脑空闲至指定时间后，将启动您选择的屏幕保护程序。
...
| ![org.deepin.browser](../common/org.deepin.browser.svg) | 浏览器 - 单击打开网页。           | ![deepin-appstore](../common/deepin-appstore.svg)        | 应用商店 - 搜索安装应用软件。                         |
...
| ![search](../common/search-dark.svg) |   全局搜索 - 搜索、查找内容。   | ![onboard](../common/onboard.svg) | 屏幕键盘 - 单击使用虚拟键盘。 |
| ![desktop-ai-assistant](../common/desktop-ai-assistant.svg) | 桌面智能助手 - 使用语音或文字来发布指令或进行询问。 | ![shutdown](../common/system-shutdown.svg) | 电源 - 单击进入关机界面。 |
| ![trash](../common/trash_icon.svg) | 回收站。 | ![Screen-Capture](../common/Screen-Capture.svg) | 截图录屏 - 点击使用截图录屏。 |
...
3. 在插件区域勾选或取消勾选 **回收站**、**系统监视器**、**电源**、**显示桌面**、**屏幕键盘**、**通知中心**、**时间**、**桌面智能助手**、**截图录屏**等插件，可以设置这些插件在任务栏上的显示或隐藏效果。
...
通过桌面智能助手可以调节系统音量、屏幕亮度、还可以开启/关闭电源节能模式、无线网络或蓝牙。
...
### 进入关机界面
单击任务栏上的 ![shutdown](../common/system-shutdown.svg) 进入关机界面，或在启动器的小窗口模式中单击 ![poweroff_normal](../common/poweroff_normal.svg) 进入关机界面。
...
> ![notes](../common/notes.svg) 说明：当系统存在多个帐户时才显示 **切换使用者** ![userswitch_normal](../common/userswitch_normal.svg)。
...
- 将鼠标悬停在应用图标上，按住鼠标左键不放，将应用图标拖曳到指定的位置自由排列。
...
- 返回按钮 ![back](../common/back.svg)：单击即可返回首页。
- 搜索框：输入关键字，回车，搜索相应设置。
- 主菜单  ![menu](../common/icon_menu.svg) ：单击打开主菜单。在主菜单中，可以设置窗口主题、查看版本或退出控制中心。


### 生物认证|authentication

生物认证又称生物识别，是通过电脑利用人体所固有的生理特征或行为特征来进行个人身份鉴定。常用的生物认证有指纹识别、人脸识别和虹膜识别。

当接入具备生物认证的硬件设备后，在系统中录入指纹、人脸、虹膜信息，可以用来登录系统、解锁屏幕、授权特殊操作，还可以增强用户体验、提升系统使用效率、提高系统安全性。

> ![notes](../common/notes.svg) 说明：只有接入具备生物认证的硬件设备后才显示生物认证模块，才可以进行指纹、人脸、虹膜密码设置。
...
1. 在控制中心首页，单击 ![biometric_authentication](../common/biometric_auth.svg)。
3. 单击 **指纹**，在指纹密码区域单击 **添加指纹** 。
3. 在授权对话框输入当前帐户的密码后，使用指纹设备录入指纹。
4. 待指纹添加成功后单击 **完成**。
...
#### 设置人脸识别
1. 在控制中心首页，单击 ![biometric_authentication](../common/biometric_auth.svg)。
2. 单击 **人脸**，在人脸管理区域单击 **添加人脸**。
3. 在弹出的添加人脸数据对话框中，勾选 **我已阅读并同意《用户免责声明》**，并单击 **下一步**。
4. 单击 **同意并开始录入人脸**，单击 **下一步** 使用人脸识别设备录入人脸数据。
5. 单击 **完成**。
...
1. 在控制中心首页，单击 ![biometric_authentication](../common/biometric_auth.svg)。
2. 单击 **虹膜**，在虹膜管理区域单击 **添加虹膜**。
3. 在授权对话框输入当前帐户的密码。
4. 在免责声明窗口，勾选 **我已阅读并同意《用户免责声明》**，并单击 **下一步**。
5. 单击 **同意并开始录入虹膜**，单击 **下一步** 使用虹膜设备录入虹膜数据。
6. 单击 **完成**。
...
3. 选择帐户类型，设置用户名、全名、密码、重复密码、密码提示信息。
4. 单击 **创建**。
5. 在授权对话框输入当前帐户的密码，新帐户就会添加到帐户列表中。

> ![tips](../common/tips.svg) 窍门：您还可以将标准用户的帐户类型设置为管理员。
...
若忘记密码且输入错误密码5次后，系统登录界面将触发重置密码。

**前提条件**：已绑定 Union ID。

1. 单击重置密码按钮后，在弹出的 Union ID 重置密码窗口中，设置手机号/邮箱、新密码、重复密码、密码提示、获取验证码，单击 **重置**。
2. 在弹出的信息确认对话框中，单击 **确认重置**，即可使用新密码登录系统。
...
1. 在控制中心首页，单击 ![account_normal](../common/account_normal.svg)。
2. 单击当前帐户。
3. 打开 **无密码登录** 开关，在弹出的授权对话框中输入当前帐户密码并确定后，开启无密码登录系统。
...
Union ID 注册成功并登录后，才能使用 Union ID 相关的功能。

如果您尚未注册 Union ID，可以通过以下方式进行注册。

- 进入统信官方网站进行注册（支持微信扫描、手机号码等注册方式）。
- 控制中心 Union ID 界面，单击 **登录**，弹出 Union ID 登录界面，单击 **注册**，弹出 Union ID 注册界面进行注册（支持手机号码和微信注册方式）。

登录 Union ID 后，您还可以使用云同步、应用商店、邮件客户端、浏览器等应用的相关云服务功能。

> ![tips](../common/tips.svg) 窍门：当 Union ID 绑定了本地帐户后，可以使用 Union ID 重置密码。

开启云同步功能可自动同步各种系统配置到云端，如网络、声音、鼠标、更新、任务栏、启动器、壁纸、主题、电源等。若想在另一台电脑上使用相同的系统配置，只需登录此 Union ID 帐户，即可一键同步相应的配置到设备。
...
当设备或配置支持色温功能时，才显示色温模块，您可以进行如下操作。

1. 在控制中心首页，单击 ![display_normal](../common/display_normal.svg)。
2. 在色温区域，可以：
   - 打开 **自动调节色温** 开关，进入护眼模式，自动调节色温。
...
1. 在控制中心首页，单击 ![default_applications_normal](../common/default_applications_normal.svg)。
2. 选择文件类型进入默认程序列表。
3. 单击列表下的 **添加默认程序** 按钮 ![add](../common/add.svg)，选择 desktop 文件（一般在/usr/share/applications)，或特定的二进制文件。
...
1. 在控制中心首页，单击 ![personalization_normal](../common/personalization_normal.svg)。
2. 单击 **通用**，开启 **窗口特效**。
3. 拖动 **窗口圆角** 的滑块，调节半径大小，可实时预览圆角效果。
...
在"个性化 > 任务栏"设置界面，可以对任务栏的模式、位置、状态等进行设置。

1. 在控制中心首页，单击 ![personalization_normal](../common/personalization_normal.svg)。
2. 单击 **任务栏**，进入任务栏设置界面，您可以：
   -  选择任务栏模式为高效模式或时尚模式。
   -  设置位置为上、下、左或右。
   -  设置显示状态为一直显示、一直隐藏或智能隐藏。
   -  通过拖曳任务栏大小滑块改变任务栏高度。
   -  勾选或取消勾选插件区域的插件，设置其在任务栏上的显示和隐藏效果。
...
开启飞行模式将关闭无线网络、个人热点和蓝牙功能。您可以在任务栏快速开启或者关闭飞行模式。

1. 在控制中心首页，单击  ![network_normal](../common/network_normal.svg)。
2. 单击 **飞行模式**。
3. 打开 **飞行模式** 开关，开启飞行模式。

>![notes](../common/notes.svg) 说明：如果您的电脑没有无线网卡，也不支持蓝牙，系统将不会显示飞行模式模块。
...
1. 在控制中心首页，单击  ![network_normal](../common/network_normal.svg)。
2. 单击 **VPN**，选择 **添加 VPN** 按钮 ![add](../common/add.svg) 或 **导入 VPN** 按钮![import](../common/import.svg)。
...
统信 UOS PC 与非 UOS PC 或者统信 UOS PC 与移动设备（手机/平板）之间通过蓝牙设备传送/接收文件。

前提条件：发送端蓝牙设备与接收端蓝牙设备已配对并连接成功。

**使用UOS PC端蓝牙传送文件**

场景：发送端（统信 UOS PC）通过蓝牙向接收端（非 UOS PC /手机/平板）传送文件。


1. 在控制中心首页，单击 ![bluetooth_normal](../common/bluetooth_normal.svg)。
2. 开启 **蓝牙**，系统将自动扫描附近的蓝牙设备。
3. 进入“我的设备”选择已配对连接的蓝牙设备，单击已连接附近的按钮 ![next](../common/next.svg) 进入详情，单击 **发送文件** 按钮（仅 PC/手机/平板的蓝牙设备有该按钮)，在弹出的文件管理器窗口中选择要发送的文件，单击 **打开**。
4. 在弹出的 “蓝牙文件传输” 窗口中，选择接收此文件的蓝牙设备，单击 **发送**，发送传输文件请求。

**使用手机端蓝牙传送文件**

场景：发送端（以安卓手机为例）通过蓝牙向接收端（统信 UOS PC）传送文件。
...
1. 在控制中心首页，单击 ![power_normal](../common/power_normal.svg)。
2. 单击 **通用**。
   - 打开 **显示剩余使用时间及剩余充电时间**，显示剩余电量及剩余可用时间。
   - 关闭 **显示剩余使用时间及剩余充电时间**，则仅显示剩余电量。
...
若您使用的是笔记本电脑，可以在 **鼠标 > 通用** 界面开启 **输入时禁用触控板**，或在 **鼠标 > 鼠标** 界面开启 **插入鼠标时禁用触控板**，以避免误触触控板。
...
> ![notes](../common/notes.svg) 说明：在输入法管理界面，单击 **编辑** 后，单击 **删除** 按钮![delete](../common/delete.svg)即可移除某输入法。对于被移除（非卸载）的输入法，可以在添加输入法界面进行添加。
...
7. 单击某个快捷键后的删除按钮 ![delete](../common/delete.svg)， 删除自定义的快捷键。
...
- 开启/关闭语音唤醒：当开启语音唤醒方式时，支持勾选 OneShot，开启“唤醒命令+语义命令”唤起方式，例如：你好，小华！北京天气怎样？
- 设置语言：中文-普通话
...
在更新设置页面开启可从仓库更新类型中的“系统”、或“仅安全更新”后，自动检查、自动下载、自动安装、更新提醒才能正常操作。

系统存在更新时会在控制中心首页进行提示，单击 **更新** 按钮 ![update](../common/update.svg) 进入 **检查更新** 页面，单击 **检查更新**，检查完毕后，即可下载和安装更新。除此之外，在更新设置模块中还可以设置自动下载更新。

> ![tips](../common/tips.svg) 窍门：如果不需要自动检查更新，不想收到系统或应用的更新提示，可在“更新设置”中关闭“自动检查”或“更新提醒”。
...
2. 单击 **检查更新**，进入检查系统更新界面。
   ...
   - 如果系统存在更新，将显示 **下载并安装** 按钮、**全部更新** 按钮、下载数据大小和更新详情。您可以继续执行步骤3和步骤4。
3. 单击 **下载并安装** 按钮或 **全部更新** 按钮，系统将自动开始下载并安装更新。
4. 更新完成后，会出现”更新成功”提示。您可以根据实际需要执行重启、关机或取消操作。
...
> ![notes](../common/notes.svg) 说明：当开启“仅安全性更新”时，系统只会进行安全漏洞和兼容性相关的更新。
...
> ![attention](../common/attention.svg) 注意： 只有在试用期或被激活成功的系统，才能正常使用统信 UOS 系统的所有功能，否则会有部分功能受到限制。
...
3. 单击 **立即激活**，弹出二次确认框，单击 **确定**，弹出试用期激活成功界面，此时任务栏图标变为 ![authorize2](../common/authorize2.svg)。

系统成功激活后，可以查看激活状态、试用到期时间等信息，也可以选择通过输入序列号、导入激活文件来激活系统。
...
2. 单击 **输入序列号**，输入或复制正确的序列号，单击 **立即激活**。
   > ![notes](../common/notes.svg) 说明：统信 UOS 既支持16位的序列号，也支持25位的序列号。
...
2. 单击 **导入激活文件**，弹出文件管理器，选择 .key 格式的激活文件。
...
3. 使用手机扫描二维码，手机端弹出激活界面，界面显示当前机器ID及序列号。

   <img src="fig/activate_offline2.png" alt="0|activate_offline2" style="zoom:45%;" />
   
4. 单击 **立即激活**，弹出二次确认框，单击 **确定**。

5. 跳转到激活成功界面，显示机器ID、序列号和离线激活码。

   <img src="fig/activate_offline3.png" alt="0|activate_offline3" style="zoom:45%;" />
...
统信 UOS 支持从 Grub、控制中心进行数据还原。

##### 从Grub进入还原

在开机时，在选择系统界面，单击 **系统还原**，进入统信 UOS 系统还原。用户可以选择“恢复出厂设置”读取安装时的初始化备份，也可以选择“自定义恢复”恢复已创建的历史还原点。

##### 通过控制中心进行还原

用户既可以恢复出厂设置，也可以自定义还原前期备份的数据。
...
2. 选择 **备份还原** > **还原**，您可以选择：
   ...
   - 自定义恢复
      1. 选择自定义恢复后，单击 ![loadfile](../common/loadfile.svg) 选择要还原的文件。
      2. 单击 **开始还原**，开始还原系统。
   - 历史备份
...
当设备仅存在一个系统时，“启动延时”默认关闭，开机后在启动菜单停留1秒钟，不需要进行选择。
...
#### 开发者模式
进入开发者模式您可以使用 root 权限，执行 sudo 操作，安装和运行未在应用商店上架的非签名应用。系统默认未进入开发者模式。

> ![attention](../common/attention.svg) 注意： 进入开发者模式后的行为可能存在风险，且将不再享有官方保修服务，请谨慎操作。

在控制中心首页，单击 ![general](../common/general.svg)，选择 **开发者模式**，单击 **进入开发者模式** 按钮，进入开发者模式窗口，您可以：

- 在线激活
   1. 选择 **在线激活**，单击 **下一步**，在弹出的 Union ID 登录窗口中，输入登录信息。
...
使用窗口分屏，让多个应用和任务窗口同时显示在大屏幕上，既能充分利用大屏幕的空间，又能减少多个应用窗口之间的频繁切换，提高您的工作学习效率。

#### 进入分屏

1. 拖曳窗口到屏幕左侧或右侧，当光标接触到屏幕左右侧边缘时，会出现分屏提⽰动效，动效颜色与活动色一致，释放窗口，窗口将吸附在屏幕左半边或右半边。
   
   > ![notes](../common/notes.svg) 说明：当应用支持分屏时，将光标悬浮于应用窗口的最大/还原按钮上，在弹出的菜单中选择 ![split_screen](../common/split_left_hover.svg) 或 ![split_screen](../common/split_right_hover.svg)，即可进入分屏。
   
2. 在另一半未被占据的屏幕中，会出现其他所有窗口的预览，单击其中一个使其显示在另一半屏幕中，实现分屏浏览。也可以把其他窗口拖曳到屏幕两边，叠加分屏窗口。
   
   > ![tips](../common/tips.svg) 窍门：要取消窗口分屏，只需将窗口从屏幕一侧拖离即可。

多屏分屏模式下，支持在同一块屏幕配对分屏窗口，不支持跨屏配对分屏窗口。

#### 调整窗口位置

分屏浏览时，移动窗口的1/2位置越过分屏区域中线，另一窗口自动切换到移动窗口分屏位置，释放移动窗口，移动窗口补位到空缺的分屏位置。

#### 调整窗口大小

分屏浏览时，将光标移动到窗口的拼接处进行左右拖曳，可以同步修改窗口的宽度。

> ![notes](../common/notes.svg) 说明：当窗口不能继续向某一个方向拖曳时，光标会指示其他可拖曳方向。

#### 退出分屏

- 单击当前任意窗口的最大化按钮或关闭按钮，仅该窗口退出分屏，剩余窗口不退出分屏。

- 两个窗口分屏时，拖曳其中一个窗口到屏幕顶部触发一分屏，当前操作的窗口退出分屏，剩余窗口不退出分屏。

- 拖曳窗口远离分屏窗口拼接方向，该窗口退出分屏。
