...
在開機時，在選擇系統介面，單擊 **UnionTech OS Restore**，進入統信UOS系統還原，使用者可以選擇 **復原出廠設定** 讀取還原出廠設定備份，也可以選擇 **自備份文件還原** 打開文件目錄選取要還原的備份。
...
2. 單擊 **開發者模式**，單擊 **進入開發者模式** 按鈕。
   - 若選擇線上模式，請先登入Union ID。仔細查看開發者模式免責聲明，了解注意事項後，勾選 **同意並進入開發者模式**，單擊 **確認**。待系統下發證書後，按鈕變為 **已進入開發者模式**。
