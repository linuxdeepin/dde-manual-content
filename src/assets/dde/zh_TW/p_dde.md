...
成功登入系統後，即可體驗統信 UOS 桌面環境。桌面環境主要由桌面、任務欄、啟動器、控制中心和視窗管理器等組成，是您使用該作業系統的基礎。
...
1. 單擊重設密碼按鈕後，在彈出的 Union ID 重設密碼視窗中，設定手機號碼/信箱、新密碼、重複密碼、密碼提示、獲取驗證碼，單擊 **重設**。
...
Union ID 註冊成功並登入後，才能使用 Union ID 相關的功能。

如果您尚未註冊 Union ID，可以透過以下方式進行註冊。

- 進入統信官方網站進行註冊（支援微信掃描、手機號碼等註冊方式）。
- 控制中心 Union ID 介面，單擊 **登入**，彈出 Union ID 登入介面，單擊 **註冊**，彈出 Union ID 註冊介面進行註冊（支援手機號碼和微信註冊方式）。

登入Union ID後，您可以使用雲同步、應用商店、郵箱用戶端、瀏覽器等相關雲服務功能。

> ![tips](../common/tips.svg) 竅門：當 Union ID 綁定了本機帳戶後，可以使用 Union ID 重設密碼。
...
1. 在控制中心首頁，單擊 ![personalization_normal](../common/personalization_normal.svg)。
2. 單擊 **通用**，開啟 **視窗特效**。
3. 拖動 **視窗圓角** 的滑塊，調節半徑大小，可即時預覽圓角效果。
...
多屏擴展模式時，任務欄位置可以設定為跟隨滑鼠位置顯示，或僅主屏顯示。
...
開啟飛行模式將關閉無線網路、個人熱點和藍牙功能。您可以在任務欄快速開啟或者關閉飛行模式。

1. 在控制中心首頁，單擊  ![network_normal](../common/network_normal.svg)。
2. 單擊 **飛行模式**。
3. 打開 **飛行模式** 開關，開啟飛行模式。

>![notes](../common/notes.svg) 說明：如果您的電腦沒有無線網卡，也不支援藍牙，系統將不會顯示飛行模式模組。
...
統信 UOS 提供從 Grub 進入還原、控制中心還原的還原方式。

##### 從Grub進入還原

在開機時，在選擇系統介面，單擊 **系统还原**，進入統信 UOS 系統還原，使用者可以選擇 **復原出廠設定** 讀取還原出廠設定備份，也可以選擇 **自備份文件還原** 打開文件目錄選取要還原的備份。

##### 透過控制中心進行還原

使用者既可以還原出廠設定，也可以自訂復原還原前期備份的資料。
...
> ![notes](../common/notes.svg) 說明：arm架構系統僅支援grub驗證，mips架構系統暫不支援grub驗證、不支援更換啟動選單背景圖。
...
2. 單擊 **開機選單**，進入開機選單介面。
3. 單擊選中某一選單項後，彈出授權認證視窗，請輸入密碼授權該項，該項將成為預設啟動項。
...
#### 開發者模式
進入開發者模式您可以使用root權限，執行sudo操作，安裝和執行未在應用商店上架的非簽名應用程式。系統預設未進入開發者模式。

> ![attention](../common/attention.svg) 注意： 進入開發者模式後的行為可能存在風險，且將不再享有官方保修服務，請謹慎操作。

在控制中心首頁，單擊 ![general](../common/general.svg)，選擇 **開發者模式**，單擊 **進入開發者模式** 按鈕，進入開發者模式窗口，您可以：

- 在線啟動
   1. 選擇 **線上啟動**，單擊 **下一步**，在彈出的Union ID登錄窗口中，輸入登錄訊息。
   2. 彈出免責聲明窗口，仔細閱讀開發者模式免責聲明，了解注意事項後，勾選 **同意並進入開發者模式**，單擊 **確定**。
   3. 在彈出的授權認證窗口中輸入認證訊息。
   3. 認證通過，在彈出的是否重啟對話框中單擊 **立刻重新啟動**， 重啟系統後開發者模式生效。
- 離線啟動
   1. 選擇 **離線啟動**，根據提示下載證書，並導入證書。
   2. 在彈出的授權認證窗口中輸入認證訊息。
   3. 認證通過，在彈出的是否重啟對話框中單擊 **立刻重新啟動**， 重啟系統後開發者模式生效。
...
如果您開啟使用者體驗計劃，即您授權我們收集和使用您的裝置系統訊息，以及應用軟體訊息。這些訊息幫助改善使用者最常使用的功能，解決軟體操作過程中遇到的問題，從而更好地滿足使用者的需求、提升使用者體驗。

為了更好地使用該系統，建議使用者開啟使用者體驗計劃，該計劃可以隨時參與或退出。

1. 在控制中心首頁，單擊 ![general](../common/general.svg)。
2. 打開 **使用者體驗計畫** 開關，了解使用者體驗計劃許可協議。
...
> ![notes](../common/notes.svg) 說明：amd、arm架構系統支援域管理設定功能。
