...
初次進入統信操作系統，會自動打開歡迎程序。您可以觀看影片了解系統功能，選擇桌面樣式、運行模式和圖標主題，進一步了解該系統。
...
桌面是登錄後看到的主螢幕區域。在桌面上可以新建文件夾/文檔、排序文檔、在終端中打開、設置壁紙與屏保等，還可以通過 [傳送到桌面](#設置快捷方式) 向桌面添加應用的快捷方式。
...
1. 在桌面上，單擊鼠標右鍵。
2. 單擊 **壁紙與屏保**，選擇 **屏保**，在桌面底部預覽所有螢幕保護程序。
3. 選中一個螢幕保護程序，設置閒置時間。
...
在剪貼板上，可以將有源圖片、文件類訊息直接拖拽到桌面保存，也可以將文字、字符等訊息拖拽到可編輯文本中。
...
任務欄一般是指位於桌面底部的長條，主要由啟動器、應用程序圖標、托盤區、系統外掛等組成。在任務欄中，可以打開啟動器、顯示桌面、進入工作區，對其上的應用程序進行打開、新建、關閉、強制退出等操作，還可以設置輸入法，調節音量，連接網絡，查看日曆，進入關機界面等。

> ![notes](../common/notes.svg) 說明：在控制中心“個性化 > 任務欄”界面，也可以對任務欄的模式、位置、狀態等進行設置。
...
| ![onboard](../common/search-dark.svg) | 全局搜索 - 搜索、查找內容。 |![onboard](../common/onboard.svg) | 螢幕鍵盤 - 單擊使用虛擬鍵盤。 |
| ![desktop-ai-assistant](../common/desktop-ai-assistant.svg) | 桌面智能助手 - 使用語音或文字來發佈指令或進行詢問。 | ![shutdown](../common/system-shutdown.svg) | 電源 - 單擊進入關機界面。                         |
| ![trash](../common/trash_icon.svg) | 回收站。      |                             |                                        |

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
> ![notes](../common/notes.svg) 說明：禁用無線網絡功能時，飛行模式不可用。
...
3. 在插件區域勾選或取消勾選 **回收站**、**系統監視器**、**電源**、**顯示桌面**、**螢幕鍵盤**、**通知中心**、**時間**、**桌面智能助手** 等插件，可以設置這些插件在任務欄上的顯示和隱藏效果。

### 查看通知消息

當有系統或應用通知時，桌面上方會彈出通知消息。若有按鈕，單擊按鈕執行對應操作；若無按鈕，單擊關閉此消息。
...
您可以單擊任務欄上的 ![shutdown](../common/system-shutdown.svg)  進入關機界面，也可以在啟動器的小窗口模式中單擊 ![poweroff_normal](../common/poweroff_normal.svg) 進入關機界面。
...
- 右鍵單擊任務欄上的應用圖標，選擇 **移除駐留** 將應用從任務欄上移除。
...
> ![notes](../common/notes.svg) 說明：以上操作，只能刪除應用的快捷方式，而不會卸載應用。
...
> ![notes](../common/notes.svg) 說明：部分系統應用不能被卸載。

## 控制中心

統信操作系統通過控制中心來管理系統的基本設置，包括帳戶管理、網絡設置、日期與時刻設置、個性化設置、顯示設置、系統升級等。當您進入桌面環境後，單擊任務欄上的 ![controlcenter](../common/controlcenter.svg) 即可打開控制中心窗口。
...
生物認證又稱生物識別，是通過電腦利用人體所固有的生理特徵或行為特徵來進行個人身份鑑定。常用的生物認證有指紋識別、人臉識別和虹膜識別。

當接入具備生物認證的硬件設備後，在系統中錄入指紋、人臉、虹膜訊息，可以用來登錄系統、解鎖螢幕、授權特殊操作，還可以增強用戶體驗、提升系統使用效率、提高系統安全性。

> ![notes](../common/notes.svg) 說明：只有接入具備生物認證的硬件設備後才顯示生物認證模塊，才可以進行指紋、人臉、虹膜密碼設置。
...
3. 在彈出的添加人臉數據對話框中，勾選 **我已閱讀並同意《用戶免責聲明》**，並單擊 **下一步**。
4. 單擊 **同意並開始錄入人臉**，單擊 **下一步** 使用人臉識別設備錄入人臉數據。
5. 單擊 **完成**。

> ![notes](../common/notes.svg) 說明：一個帳戶最多可錄入5個人臉。
...
3. 在授權對話框輸入當前帳戶的密碼。
4. 在免責聲明窗口，勾選 **我已閱讀並同意《用戶免責聲明》**，並單擊 **下一步**。
5. 單擊 **同意並開始錄入虹膜**，單擊 **下一步** 使用虹膜設備錄入虹膜數據。
6. 單擊 **完成**。
...
3. 選擇帳戶類型，輸入用戶名、密碼和重複密碼，設置全名、密碼提示。
4. 單擊 **創建**。
5. 在授權對話框輸入當前帳戶的密碼，新帳戶就會添加到帳戶列表中。

> ![tips](../common/tips.svg) 竅門：您還可以將標準用戶的帳戶類型設置為管理員。
...
> ![notes](../common/notes.svg) 說明：管理員帳號過期之後需要先修改密碼才能進行授權。
...
1. 在控制中心首頁，單擊![account_normal](../common/account_normal.svg)。
2. 單擊當前帳戶。
3. 打開 **無密碼登錄** 開關，在彈出的授權對話框中輸入當前帳戶密碼並確定後，開啟無密碼登錄系統。
...
密碼有效期自修改之日起開始計算。例如：用戶A在9月1日修改了密碼後，將密碼有效期更改為30天。在這種情況下，密碼可以在9月1日至9月30日這期間的30天正常使用。
...
Union ID註冊成功並登錄後，才能使用Union ID相關的功能。

如果您尚未註冊Union ID，可以通過以下方式進行註冊。

- 進入統信官方網站進行註冊（支持微信掃描、手機號碼等註冊方式）。
- 控制中心Union ID界面，單擊 **登錄**，彈出Union ID登錄界面，單擊 **註冊**，彈出Union ID註冊界面進行註冊（支持手機號碼和微信註冊方式）。

登錄Union ID後，您還可以使用雲同步、應用商店、郵件客戶端、瀏覽器等應用的相關雲服務功能。

> ![tips](../common/tips.svg) 竅門：當Union ID綁定了本地帳戶後，可以使用Union ID重置密碼。
...
當設備或配置支持亮度調節功能時，才顯示亮度模塊，您可以進行如下操作。
...
當設備或配置支持色溫功能時，才顯示色溫模塊，您可以進行如下操作。
...
當顯卡支持桌面顯示功能且解像度設置為非推薦解像度時，您可以進行如下操作。
...
多屏螢幕亮度、色溫、螢幕縮放、解像度、刷新率、方向等的設置方式與單屏設置方式相同，詳細操作方法請參考 [單屏設置](#單屏設置)。
...
6. 當網絡連接成功後，桌面上方將彈出 **已連接有線連接** 的提示訊息。
...
通過個人熱點將計算機網絡信號轉換為WiFi熱點，以供一定距離內的其他設備無線連接。要開啟無線熱點，計算機必須裝有支持熱點功能的無線網卡。
...
3. 選擇VPN協議類型，並輸入名稱、網關、用戶名、密碼等訊息。（導入VPN會自動填充訊息）
...
> ![notes](../common/notes.svg) 說明：當手提電腦配備了藍牙時，您可以直接開啟藍牙開關；當台式電腦沒有配備藍牙時，您可以購買藍牙適配器，插入到電腦的USB端口中，再使用藍牙。
...
3. 進入“我的設備”選擇已配對連接的藍牙設備，單擊已連接附近的按鈕 ![next](../common/next.svg) 進入詳情，單擊 **發送文件** 按鈕（僅PC/手機/平板的藍牙設備有該按鈕），在彈出的文件管理器窗口中選擇要發送的文件，單擊 **打開**。
...
- 如果接收端為如下情況，發送端部分文件傳輸失敗，會顯示在傳輸失敗列表。
   ...
   - 接收端既不單擊接收也不單擊拒絕，默認1分鐘後則結束傳輸，顯示傳輸失敗。
...
1. 在控制中心首頁，單擊 ![power_normal](../common/power_normal.svg)。
2. 點擊 **通用**。
   - 打開 **顯示剩餘電量及剩餘充電時間**，顯示剩餘電量及剩餘可用時間。
...
數位板，又名繪圖板、繪畫板、手繪板等，主要用作繪畫創作，是電腦的一種輸入設備。
...
> ![notes](../common/notes.svg) 說明：在輸入法高級設置界面中，還可以設置更多快捷鍵。
...
1. 在控制中心首頁，單擊 ![keyboard_normal](../common/keyboard_normal.svg)。
2. 單擊 **快捷鍵**。
3. 單擊 **添加** 按鈕 ![add](../common/add.svg)，進入添加自定義快捷鍵界面。
4. 輸入快捷鍵名稱、命令和快捷鍵。
5. 單擊 **添加**。
...
在更新設置頁面開啟可從倉庫更新類型中的“系統”、“僅安全更新”或“第三方倉庫“後，自動檢查、自動下載、自動安裝、更新提醒才能正常操作。

系統存在更新時會在控制中心首頁進行提示，單擊 **更新** 按鈕 ![update](../common/update.svg) 進入 **檢查更新** 頁面，單擊 **檢查更新**，檢查完畢後，即可下載並安裝更新。除此之外，在更新設置模塊中還可以設置自動下載更新。

> ![tips](../common/tips.svg) 竅門：如果不需要檢查更新，不想收到系統或應用的更新提示，可在 **更新設置** 中關閉 **自動檢查** 或 **更新提醒**。
...
2. 單擊 **檢查更新**，進入檢查系統更新界面。
   ...
   - 如果系統存在更新，將顯示 **下載並安裝** 按鈕、**全部更新** 按鈕，下載數據大小和更新詳情。您可以繼續執行步驟3和步驟4。

3. 單擊 **安裝更新** 按鈕或 **全部更新** 按鈕，系統將自動開始下載並安裝更新。
...
> ![notes](../common/notes.svg) 說明：當開啟“僅安全性更新”時，系統只會進行安全漏洞和兼容性相關的更新。
...
> ![tips](../common/tips.svg) 竅門：打開 **清除軟件包緩存** 開關，系統會定期自動清理下載軟件包導致的緩存。
...
每台設備僅有一次試用期啟用機會（默認開機自動啟用），從啟用當天算起，有效期為90天。試用期間可使用系統全部功能。試用期間可使用系統全部功能，也可隨時通過序列號或啟用文件永久啟用系統。
...
2. 選擇 **備份還原** > **還原**，您可以選擇：
   ...
   - 自定義恢復
      1. 選擇自定義恢復後，單擊 ![loadfile](../common/loadfile.svg) 選擇要還原的文件。
      2. 單擊 **開始還原**，開始還原系統。
   - 歷史備份
...
> ![notes](../common/notes.svg) 說明：arm架構系統僅支持grub驗證，mips架構系統暫不支持grub驗證、不支持更換啟動菜單背景圖。
...
2. 單擊 **開發者模式**，單擊 **進入開發者模式** 按鈕。
   - 選擇在線模式，請先登錄網絡帳戶。仔細查看開發者模式免責聲明，了解注意事項後，勾選 **同意並進入開發者模式**，單擊 **確定**。待系統下發證書後，按鈕變為 **已進入開發者模式**。
   - 選擇離線模式，根據提示下載證書，待系統導入證書後，即可進入開發者模式。
3. 在彈出的對話框中單擊 **立即重啟**， 重啟系統後開發者模式生效。

> ![attention](../common/attention.svg) 注意：進入開發者模式後不可退出或撤銷。系統所有帳戶都將擁有root權限。

#### 用戶體驗計劃
如果您開啟用戶體驗計劃，即您授權我們收集和使用您的設備系統訊息，以及應用軟件訊息。這些訊息幫助改善用戶最常使用的功能，解決軟件操作過程中遇到的問題，從而更好地滿足用戶的需求、提升用戶體驗。
...
> ![notes](../common/notes.svg) 說明：amd、arm架構系統支持域管理設置功能。
...
使用窗口分屏，讓多個應用和任務窗口同時顯示在大螢幕上，既能充分利用大螢幕的空間，又能減少多個應用窗口之間的頻繁切換，提高您的工作學習效率。

#### 進入分屏

1. 拖拽窗口到螢幕左側或右側，當光標接觸到螢幕左右側邊緣時，會出現分屏提⽰動效，動效顏色與活動色一致，釋放窗口，窗口將吸附在螢幕左半邊或右半邊。
   
   > ![notes](../common/notes.svg) 說明：當應用支持分屏時，將光標懸浮於應用窗口的最大/還原按鈕上，在彈出的菜單中選擇 ![split_screen](../common/split_left_hover.svg) 或 ![split_screen](../common/split_right_hover.svg)，即可進入分屏。
   
2. 在另一半未被佔據的螢幕中，會出現其他所有窗口的預覽，單擊其中一個使其顯示在另一半螢幕中，實現分屏瀏覽。也可以把其他窗口拖拽到螢幕兩邊，疊加分屏窗口。
   
   > ![tips](../common/tips.svg) 竅門：要取消窗口分屏，只需將窗口從螢幕一側拖離即可。

多屏分屏模式下，支持在同一塊螢幕配對分屏窗口，不支持跨屏配對分屏窗口。

#### 調整窗口位置

分屏瀏覽時，移動窗口的1/2位置越過分屏區域中線，另一窗口自動切換到移動窗口分屏位置，釋放移動窗口，移動窗口補位到空缺的分屏位置。

#### 調整窗口大小

分屏瀏覽時，將光標移動到螢幕中間兩個窗口的連接處進行左右拖拽，可以同時修改兩個窗口的寬度。

> ![notes](../common/notes.svg) 說明：當窗口不能繼續向某一個方向拖曳時，光標會指示其他可拖曳方向。


#### 退出分屏

- 單擊當前任意窗口的最大化按鈕或關閉按鈕，僅該窗口退出分屏，剩餘窗口不退出分屏。

- 兩個窗口分屏時，拖曳其中一個窗口到螢幕頂部觸發一分屏，當前操作的窗口退出分屏，剩餘窗口不退出分屏。

- 拖曳窗口遠離分屏窗口拼接方向，該窗口退出分屏。